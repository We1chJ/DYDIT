...
export function AddTask({ onAdd, placeholder, goals }: AddTaskProps) {
  ...
  if (!open) {
    return (
      <button
        ...
        className="flex w-full items-center gap-2.5 rounded-md py-[5px] pl-8 pr-2 text-[14.5px] text-faint transition-colors hover:bg-[var(--hover)] hover:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
      >
  ...
  return (
    // pl-8 clears the width of a task row's drag handle plus its gap, so the
    // plus sits under the checkboxes rather than out on its own.
    <div className="rounded-md py-[5px] pl-8 pr-2">
